...
router.post('/sign-up', async (req, res, next) => {
    ...
    if(foundUser){
        return res.status(409).json({message:'Username taken please sign in or Sign up with different username'})
    }
...
/*     const regexString = '^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$'
    ...
    if (!req.body.password.match(new RegExp(regexString))) {
        return res.status(400).json({
            message: 'Password must be minimum 8 characters, include at least one letter and one number'
        })
    } */
...
router.post('/sign-in',async(req,res,next)=>{
    try{
        ...
        const payload = {
            ...
            role: foundUser.role,
            assignedPos: foundUser.assignedPos ?? null,
